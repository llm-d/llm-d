...
RUNS = [
    ("k8",                        "reasoning-3turn-infperf-baseline-det-2-final",          "#1f77b4", "o"),
    ("epp kv-scorer",      "reasoning-3turn-infperf-epp-penalty286k-kv-scorer-max-picker-no-exp-det-2-final", "#d62728", "x"),
    ("epp run-req",      "reasoning-3turn-infperf-epp-penalty286k-req-scorer-max-picker-no-exp-det-2-final", "#ff7f0e", "x"),
    ("epp token-load",      "reasoning-3turn-infperf-epp-penalty286k-modified-token-scorer-max-picker-no-exp-det-2-final", "black", "x"),
    ("epp default",            "reasoning-3turn-infperf-epp-max-score-det-2-final", "#2ca02c", "x"),
    ("latency-predictor",  "reasoning-3turn-infperf-epp-penalty5s-latency-predictor-det-4-final", "#9467bd", "x"),
]
...
plt.suptitle(" vs ".join(label for label, _, _, _ in RUNS if label in [r[0] for r in runs]) + "   (inference-perf reasoning runs)", fontsize=12)
plt.tight_layout()
import os
out = os.path.join(os.path.dirname(os.path.abspath(__file__)), "baseline_vs_epp_reasoning_runs.png")
