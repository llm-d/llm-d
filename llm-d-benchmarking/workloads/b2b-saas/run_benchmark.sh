...
# but we keep the loop structure for multiple runs or future variations.
CONCURRENCY_LEVELS="${CONCURRENCY_LEVELS:-1}"
...
for scenario in "${SCENARIOS[@]}"; do
  ...
  echo "[3/4] Submitting inference-perf Jobs"
  for conc in $CONCURRENCY_LEVELS; do
    ...
    echo "    → seed=$SEED  target=$BASE_URL  job=$job_name"
