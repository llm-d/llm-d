...
3. **External Provider API Keys**:
   - A Google AI Studio API key for `gemini/gemini-3.5-flash`.
   - An [OrcaRouter](https://www.orcarouter.ai) API key for `orcarouter/auto` (smart-routing gateway).
...
kubectl -n "$NAMESPACE" create secret generic litellm-env \
  ...
  --from-literal=ORCAROUTER_API_KEY="$ORCAROUTER_API_KEY"
```

> [!IMPORTANT]
> Replace `<your-gemini-api-key>` and `<your-orcarouter-api-key>` with your actual Google AI Studio and [OrcaRouter](https://www.orcarouter.ai) API keys.
...
proxy_config:
  ...
  model_list:
    ...
    # ─────────────────────────────────────────────────────────────────
    # 4) EXTERNAL API: OrcaRouter
    # Routes through the OrcaRouter gateway, which screens every prompt/
    # response and governs tool calls on a default-deny basis. Uses the
    # API key injected via ORCAROUTER_API_KEY.
    # ─────────────────────────────────────────────────────────────────
...
> - **OrcaRouter (External Gateway)**: The `orcarouter-auto` model is routed through the [OrcaRouter](https://www.orcarouter.ai) gateway (`https://api.orcarouter.ai/v1`), so `model_info` pricing and budgets are handled by LiteLLM based on token usage, exactly as with `gemini-flash`.
...
KEY_RESP=$(curl -s http://127.0.0.1:4000/key/generate \
  ...
  -d '{
    "models": ["qwen/qwen3-32B", "gemini-flash", "orcarouter-auto"],
    "max_budget": 10.0,
    "rpm_limit": 60,
    "duration": "30d",
...
#### C. Call External Model (`orcarouter-auto` via the OrcaRouter gateway)
...
Expected output (translated OpenAI format returned through OrcaRouter):
