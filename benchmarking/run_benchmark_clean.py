...
def save_results_to_dir(results, results_dir, run_id):
...
def _patched_evaluate(self, *args, **kwargs):
    ...
    def intercept_job_creation(self, namespace, body, **kwargs):
        ...
        if job_name.startswith("lmbenchmark-evaluate"):
            ...
            # Add anti-affinity terms for all evaluation jobs using more precise matching
            anti_affinity_term = {
                "weight": 100,
                "podAffinityTerm": {
                    "labelSelector": {
            ...
            if created_manifest["spec"]["template"]["spec"].get("volumes"):
                for vol in created_manifest["spec"]["template"]["spec"]["volumes"]:
                    ...
                    elif not any(pvc_name.endswith(suffix) for suffix in ["-results-pvc", "-results"]):
                        volumes_to_keep.append(vol)
            ...
            for container in created_manifest["spec"]["template"]["spec"].get("containers", []):
                if container.get("volumeMounts"):
                    ...
                    for mount in container["volumeMounts"]:
                        if not any(mount["mountPath"].endswith(suffix) for suffix in ["-requests", "requests"]):
...
def main():
    logger.info("Starting benchmark run - VERSION: 2025-05-22-CLEAN")
    env_vars = os.environ
    ...
    logger.info(f"Using placeholder directory {placeholder_dir} to avoid PVC multi-attach")
    ...
    run_id = f"{stack_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    try:
        ...
        # Save results to the placeholder directory
        if results:
            logger.info(f"Saving results to placeholder directory: {placeholder_dir}")
            save_results_to_dir(results, placeholder_dir, run_id)
            logger.info(f"Results saved to placeholder directory")
            
            # List contents of placeholder directory
            logger.info(f"\nContents of {placeholder_dir} directory:")
            list_directory_contents(placeholder_dir)
            
            # If the actual results directory is available, try to copy results
            if os.path.exists(results_dir) and os.access(results_dir, os.W_OK):
                try:
                    logger.info(f"PVC is available. Copying results from {placeholder_dir} to {results_dir}")
                    # Create a directory for this run
                    real_run_dir = os.path.join(results_dir, f"run_{run_id}")
                    os.makedirs(real_run_dir, exist_ok=True)
                    
                    # Copy the results using subprocess
                    copy_cmd = ["cp", "-r", f"{placeholder_dir}/.", real_run_dir]
                    subprocess.run(copy_cmd, check=True)
                    logger.info(f"Successfully copied results to {real_run_dir}")
                except Exception as e:
                    logger.error(f"Failed to copy results to PVC: {e}")
            else:
                logger.info(f"PVC not available. Results only saved to placeholder: {placeholder_dir}")
