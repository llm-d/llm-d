...
- **CPU+NVMe tiered KV offloading** — CPU memory keeps evicted prefixes reusable after HBM
  fills. Local NVMe supplies a lower tier when needed; the measurements in the blog record
  CPU restores and no NVMe reads.
...
If you are throughput-bound rather than TTFT-bound, `p2w2d2w2` (2 prefill + 2 decode, also
...
nodes and NVLink within a node. Limited-input-length runs admit requests below 142K tokens;
full-input-length runs admit longer contexts. Compare absolute throughput only within the same
test.
...
| `max-model-len-130k` | prefill + decode | Sets `max-model-len` to 130000 (requires offloading; without it, `max-model-len` caps at ~108K on H200) |
