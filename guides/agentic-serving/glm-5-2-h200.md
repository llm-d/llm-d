...
nodes and NVLink within a node. Limited-input-length runs filter aiperf to only use requests
below 142K tokens. We use this for apples-to-apples comparisons with EP8 deployments that
cannot handle the entire dataset due to some entries containing close to 1M tokens.
