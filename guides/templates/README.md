...
Three scripts in [`scripts/`](../../scripts) validate and render between them.
...
# validate + render
scripts/guide-check-yaml.py    guides/my-guide/guide.yaml
scripts/guide-render.py        --yaml guides/my-guide/guide.yaml --readme guides/my-guide/README.md
scripts/guide-check-readme.py  --yaml guides/my-guide/guide.yaml --readme guides/my-guide/README.md
```
...
```bash
scripts/guide-render.py --yaml guides/optimized-baseline/guide.yaml --readme guides/optimized-baseline/README.md
```

**Step 4** — verify:

```bash
scripts/guide-check-readme.py --yaml guides/optimized-baseline/guide.yaml --readme guides/optimized-baseline/README.md
```
...
- **`skip_in:`** — a list of context tags. `[ci]` wraps the step's rendered fence in `<!-- llm-d-cicd:skip start --> … <!-- llm-d-cicd:skip end -->` so the README-parsing CI runner skips it (used above for `clone` and `secrets`, which CI already handles out-of-band).
...
All three live in [`scripts/`](../../scripts) and take absolute or relative paths.

### `guide-check-yaml.py` — validate the YAML

```bash
scripts/guide-check-yaml.py guides/optimized-baseline/guide.yaml
```
...
### `guide-render.py` — fill the README from the YAML
...
scripts/guide-render.py --yaml guides/optimized-baseline/guide.yaml --readme guides/optimized-baseline/README.md --dry-run

# CI check — non-zero if the README is out of date
scripts/guide-render.py --yaml guides/optimized-baseline/guide.yaml --readme guides/optimized-baseline/README.md --check
```

Only touches content between paired markers. Idempotent.

### `guide-check-readme.py` — validate the README
...
Run all three on every guide edit:

```bash
scripts/guide-check-yaml.py    guides/<name>/guide.yaml
scripts/guide-render.py        --yaml guides/<name>/guide.yaml --readme guides/<name>/README.md --check
scripts/guide-check-readme.py  --yaml guides/<name>/guide.yaml --readme guides/<name>/README.md
```

`--check` on the renderer catches out-of-date READMEs — someone edited `guide.yaml` without re-rendering.
...
env:                            # required
  static:                       # required — variable declarations
    ...
    VAR: { default: PLACEHOLDER, sensitive: true } # secret — compiler fails if unset
...
verify:                         # optional
  endpoint:                     # required if verify is present — one entry per mode
    <mode>:
      - run: <bash>
  tests:                        # optional
    - run: <bash>
