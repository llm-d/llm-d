...
* **An 8-rail RDMA fabric** with a Multus attachment per rail, 8 `amd.com/vnic`
  ...
  `base/prefill.yaml` lists every variable involved and what it does. Note the rail-only
  settings are part of that: without them MoRI builds a full cross-node mesh, which hangs at
  init on a rail-isolated fabric.
...
For another cluster, copy `amd-ci/` and edit it — at minimum the NAD name and namespace, and
the RoCE service level and traffic class. It leaves `MORI_RDMA_DEVICES` unset, which is right
wherever the pod only sees its own rails; set it if yours also exposes NICs MoRI must not use.
