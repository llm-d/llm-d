...
Each path is a self-contained deployment using a specific offloading implementation. Pick one and follow its deploy block under [Installation Instructions](#2-deploy-the-model-server).
...
The **vLLM native** path is the recommended default. Its CPU tier is low-overhead and enabling it is appropriate in almost all deployments; its filesystem tier uses vLLM's native multi-tier offloading (`TieringOffloadingSpec`) to spill from CPU RAM to shared storage (HBM → CPU RAM → filesystem).
...
Deploy **one** of the paths below. Each `kubectl apply -k` targets an overlay directory; the `INFRA_PROVIDER` selects a `base` overlay or a provider-specific one (for example `gke`).
...
```bash
# Set these to the path you deployed in step 2.
...
The results below show the effect of enabling prefix-cache offloading relative to an HBM-only configuration, under a high-cache scenario where the working set exceeds HBM but fits within HBM + CPU RAM. The weight configuration defaults to `1:1:1:1:1` (Queue Scorer : KV Cache Utilization Scorer : GPU Prefix Cache Scorer : CPU Prefix Cache Scorer : LRU Scorer).
...
The benchmark runs on 16 × H100 GPUs, distributed across 16 model servers (1 H100 per server with TP=1) using gpt-oss-120B and the same workload as the GPU results above. For detailed results see [gpt-oss-120B benchmarking results](benchmark-results-gpt-oss-120b.md).
