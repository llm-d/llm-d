# Inference Scheduling - Scheduler - REMOVE THIS README LATER

This guide adds a single override: the `llm-d.ai/guide: inference-scheduling` match label so the scheduler discovers the correct model server pods.
...
```bash
helm install inference-scheduling-scheduler \
  oci://registry.k8s.io/gateway-api-inference-extension/charts/standalone \ 
  -f guides/recipes/scheduler/base.values.yaml \
  -f guides/recipes/scheduler/features/monitoring.values.yaml \
  -f guides/inference-scheduling/scheduler/inference-scheduling.values.yaml \
