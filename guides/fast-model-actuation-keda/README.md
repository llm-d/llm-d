...
This creates 1 [server-requesting pod](../fast-model-actuation/README.md#overview), which reserves a GPU, and the FMA controllers bind it to a [launcher pod](../fast-model-actuation/README.md#overview) — the launcher is what actually runs the vLLM instance. In [step 6](#6-enable-scale-from-zero-autoscaling-keda) KEDA takes over the `fma-requester` replica count and scales the requesting pod away once idle, releasing the GPU and putting the vLLM instance to sleep; the launcher stays running.
...
kubectl run keda-epp-warmup -n ${NAMESPACE} --rm -i --quiet \
  ...
  --env="URL=http://${GUIDE_NAME}-epp/v1/completions" \
  --env="MODEL=${MODEL}" \
  -- /bin/sh -c '
    for i in 1 2 3; do
      curl -sS --max-time 60 -o /dev/null -w "  warmup ${i}: HTTP %{http_code}\n" \
...
> [!NOTE]
> This scale-up should trigger a [hot start](../fast-model-actuation/README.md#overview) — the fast path, waking the vLLM instance that step 6 put to sleep, at 4.0 s mean pod startup on `Qwen/Qwen3-32B` against 85.3 s for a warm start ([benchmark results](./benchmark-results/qwen3-32b-h100/README.md#queue-based-autoscaling)). Which path you get depends on GPU assignment; see [wake latency](../fast-model-actuation/README.md#3-demonstrate-sleepwake) in the FMA guide.
