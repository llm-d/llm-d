...
| Intel XPU            | `modelserver/xpu/vllm/`    | Qwen/Qwen3-32B                          | B60-class deployment uses two replicas, tensor parallelism 2, `--quantization=fp8`, and `--max-model-len=24576`; FP8 KV cache is not enabled on XPU FlashAttention. Users with a node-local model cache can uncomment the optional `/data/hf_models` hostPath in the overlay. |
