...
- **ModelService - Declarative Stack for Model Deployment**: ModelService is intended to be an easy (but not the only) way for model owners to deploy models on llm-d. It provides a custom resource that declaratively provisions and manages all Kubernetes resources needed to serve all variants of a given model. In particular, it orchestrates disaggregated prefill and decode deployments, routing components (InferenceModel, InferencePool, Endpoint Picker), and the requisite RBAC permissions. Through integration with the Gateway API Inference Extension and support for Hugging Face, PVC, and OCI-based models, ModelService offers a unified interface for model onboarding. It supports independent scaling of prefill and decode, flexible hardware scheduling, value templating, and semantic merging via reusable BaseConfig presets—enabling platform operators to enforce shared defaults while giving model owners fine-grained control.

See [ModelService repo](https://github.com/llm-d/llm-d-model-service) for model details. Our plans for ModelService include support for LoRA adapters and routing weights through better integration with Gateway API Inference Extension, multi-node inferencing using [LeaderWorkerSet](https://github.com/kubernetes-sigs/lws), and a CRD for BaseConfig presets. 
