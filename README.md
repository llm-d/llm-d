...
- [2026-02] The [v0.5](https://llm-d.ai/blog/llm-d-v0.5-sustaining-performance-at-scale) release focuses on sustaining performance at scale with hierarchical KV offloading (GPU/CPU/filesystem tiers) to decouple cache capacity from GPU memory, cache-aware LoRA routing for multi-tenant workloads, resilient networking via the new UCCL backend showing 2.4x better resilience under network congestion, and scale-to-zero autoscaling for cost-efficient intermittent workloads. Benchmarks demonstrate ~50k output tokens/sec on B200 for throughput workloads and up to 109% higher throughput with 99% lower TTFT versus baseline Kubernetes service for inference scheduling.
