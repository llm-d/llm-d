...
* Only `llm-d-workload-variant-autoscaler` implements `OTEL_TRACES_EXPORTER=none|otlp|console` as a clean per-process on/off switch.
...
* **llm-d-router**: audit `InitTracing` (`pkg/common/observability/tracing/telemetry.go`) to confirm the propagator is installed before the exporter/sampler is resolved, so propagation survives `OTEL_TRACES_EXPORTER=none`; add the `scopeFilterProcessor` and `OTEL_TRACES_ENABLED_SCOPES` wiring, scoped to the EPP binary (`cmd/epp`); no change needed for `cmd/pd-sidecar`, which already gets independent process-level control.
